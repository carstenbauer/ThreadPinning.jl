...
function _hwloc_package_puids()
    ...
    package_puids = [
        getproperty.(Hwloc.collectobjects(:PU, pkg), :logical_index) for pkg in packages
    ]
...
function _visualize_affinity(;
    thread_cpuids=getcpuids(), blocksize=32, color=true, ht=hyperthreading_is_enabled()
)
    package_puids = _hwloc_package_puids()
            ...
            if color
                if puid in thread_cpuids
                    printstyled(
                        puid;
                        bold=true,
                        color=(ht && isodd(puid)) ? :light_magenta : :yellow,
                    )
                else
